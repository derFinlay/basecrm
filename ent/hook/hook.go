...
// The TelFunc type is an adapter to allow the use of ordinary
// function as Tel mutator.
type TelFunc func(context.Context, *ent.TelMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TelFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TelMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TelMutation", m)
}
